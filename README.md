...
Production-oriented Go backend foundation for football data: competitions, seasons, teams, venues, players, coaches, fixtures, live matches, lineups, events, and standings.
...
`make seed-demo` creates one league, one current season, one venue, four teams, four players, two coaches, one official, standings with home/away splits, lineups/team formations, player match statistics, and three matches (live, scheduled, and finished). Running it again resets the same fixed-ID fixtures without creating duplicates. The command refuses to run when `SSB_ENV=production`.
